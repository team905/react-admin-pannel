          ...
          <div className="flex gap-2 sm:items-center md:gap-4">
            ...
            <div className="sm:flex sm:flex-col">
              ...
              <ul className="flex items-center divide-x">
                {
                  flight.meta && Object.entries(flight.meta).map(([key, value]) => {
                    return (
                      <li key={key} className="hidden px-1.5 md:block first:block first:ps-0">{value}</li>
                    )
                  })
                }
              </ul>
    ...
    >
      ...
      <div className="flex items-center justify-between gap-4 bg-gray-50 px-5 py-3 text-gray-500 dark:bg-gray-100">
        ...
        <ul className="flex shrink-0 grow-0 basis-auto items-center gap-4">
          {
            flight.bucket && Object.entries(flight.bucket).map(([key, value]) => {
              return (
        ...
        <div className="p-3 @xl:p-4 @2xl:p-5 xl:col-span-7">
          <div className="grid grid-cols-8">
            <div className="hidden flex-col gap-0.5 lg:col-span-2 lg:flex">
              <Text as='span' className="text-sm font-semibold text-gray-900">{flight.routes?.departureTime as string}</Text>
            ...
            <div className="col-span-7 flex flex-col pb-9 lg:col-span-5">
              <Text as='span' className="block text-xs font-semibold text-gray-900 lg:hidden lg:text-sm">
                {flight.routes?.departureTime as string}
              </Text>
              <Text as='span' className="font-semibold text-gray-900">{flight.routes?.departureCity}</Text>
          ...
          {flight.routes?.layover?.map((layover, index) => (
            <div key={index} className="grid grid-cols-8">
              ...
              <div className="relative col-span-1 mt-1.5">
                <Text as='span' className="relative z-10 mb-1.5 block h-3 w-3 rounded-full before:absolute before:-left-1.5 before:-top-1.5 before:-z-10 before:h-6 before:w-6 before:rounded-full before:bg-white dark:bg-gray-900 dark:before:bg-gray-50">
                  <PlaneIcon className="h-3 w-3" />
                </Text>
                <Text as='span' className="absolute left-[5px] top-[18px] block h-full w-[2px] bg-gray-200"></Text>
              </div>
              <div className="col-span-7 flex flex-col pb-9 lg:col-span-5">
          ...
          <div className="grid grid-cols-8">
            <div className="hidden flex-col gap-0.5 lg:col-span-2 lg:flex">
              <Text as='span' className=" font-semibold text-gray-900">{flight.routes?.arrivalTime as string}</Text>
              <Text as='span'>{flight.routes?.departureDate as string}</Text>
            </div>

            <div className="relative z-10 col-span-1 mt-1.5">
              <Text as='span' className="relative -bottom-[38px] block h-3 w-3 rounded-full bg-gray-900 before:absolute before:-left-1.5 before:-top-1.5 before:-z-10 before:h-6 before:w-6 before:rounded-full before:bg-gray-0 dark:bg-gray-900 dark:before:bg-gray-50 lg:-bottom-[22px]"></Text>
              <Text as='span' className="absolute -top-[2px] left-[5px] -z-20 block h-full w-[2px] bg-gray-200"></Text>
            </div>
            ...
            <div className="col-span-7 flex flex-col lg:col-span-5">
              ...
              <Text as='span' className=" font-semibold text-gray-900">{flight.routes?.arrivalCity as string}</Text>
              <Text as='span' className="text-sm text-gray-500">
